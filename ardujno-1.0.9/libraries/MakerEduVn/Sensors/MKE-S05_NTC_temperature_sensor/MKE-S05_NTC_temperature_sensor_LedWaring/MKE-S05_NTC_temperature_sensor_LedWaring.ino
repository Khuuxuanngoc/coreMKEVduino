...
int lowValueWarning             = 290;              // Set value of LOW Level
int highValueWarning            = 310;              // Set value of HIGH Level
int sensorValue                 = 0;                // Declare value variable
void setup(){
    Serial.begin(9600);                 // Baudrate 9600
    Serial.println("MakerEdu.vn Demo Read MKE-S05_NTC_temperature_sensor And Control Led");
    pinMode(led_Warning_Pin, OUTPUT);   // Cấu hình chân gắn Module led là OUTPUT
}
void loop(){
    // Read value (0->1023) from analogPin (A1)
    sensorValue = analogRead(ntc_sensor_pin);
